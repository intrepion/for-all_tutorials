...
Instructions for building `lighting-studio` from the spec using one setup path and separate core and adapter walkthroughs.
...
For this project, the core repo should expose `clamp_lighting_controls` and `build_lighting_scene`, and the adapter repo should provide sliders and environment selection by delegating ranges and fallback behavior to those core contracts.
