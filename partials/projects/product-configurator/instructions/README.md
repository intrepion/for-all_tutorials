...
Instructions for building `product-configurator` from the spec using one setup path and separate core and adapter walkthroughs.
...
For this project, the core repo should expose `build_variant_catalog`, `find_variant_by_exact_id`, and `build_variant_scene`, and the adapter repo should swap models by delegating variant lookup and default scene values to those core contracts.
