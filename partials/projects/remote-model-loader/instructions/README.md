...
Instructions for building `remote-model-loader` from the spec using one setup path and separate core and adapter walkthroughs.
...
For this project, the core repo should expose `calculate_load_percentage`, `format_loading_message`, `format_model_load_error`, and `can_interact_with_model`, and the adapter repo should map remote loading callbacks onto those core-owned rules.
