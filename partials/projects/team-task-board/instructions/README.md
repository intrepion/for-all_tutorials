...
Instructions for building `team-task-board` from the spec using one setup path and separate core and adapter walkthroughs.
...
For this project, the core repo should expose `parse_team_task_storage`, `find_task_by_id`, `filter_visible_tasks`, `can_delete_task`, `remove_task_by_id`, `format_team_task_list`, and `serialize_team_task_storage`, and the adapter repo should manage the chosen authentication, interaction flow, and durable storage mechanism by delegating task parsing, visibility, authorization, formatting, and serialization to those core functions.
